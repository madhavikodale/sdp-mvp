...
export function getPermissionsForOrgRole(role: OrganizationRole | string): Permission[] {
  const normalized = role === "admin" || role === "owner" ? "admin" : "member";
  return [...ORGANIZATION_ROLES[normalized as OrganizationRole].permissions];
}
...
export const DASHBOARD_FEATURE_FLAGS_DEFAULTS: DashboardFeatureFlags = {
  paymentsV2: false,
};
